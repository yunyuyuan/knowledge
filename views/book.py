...
def mdf_book(conn):
    ...
    if data['id'] == '新增':
        cursor.execute('insert into book(nm, author, summary, c_tm, m_tm, expl, cover, info) VALUES("%s", "%s", "%s", current_time(), current_time(), "%s", "%s", "%s")'
                       % (nm, author, summary, expl, '', info))
        conn.commit()
        ...
        book_id = int(data['id'])
        cursor.execute('update book set nm="%s",author="%s",summary="%s",m_tm=current_time(),expl="%s",info="%s" where id=%d'
                       % (nm, author, summary, expl, info, book_id))
        re = gen_response('suc', {})
